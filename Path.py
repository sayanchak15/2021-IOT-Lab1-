...
'''
We have used https://github.com/BaijayantaRoy/Medium-Article/blob/master/A_Star.ipynb for this A* algorithm as our base.
'''
...
def astar(maze, start, end, stop):
    ...
    if stop:
        ...
        if x >= 0 and y >= 0:
            maze[x][y] = 1
        # print(maze)
    path = np.array(search(maze,cost, cur, end))
    # print("New Path: ", path)
    prev = cur
    cur = np.where(path==1)
...
if __name__ == '__main__':
    ...
    try:
        ...
        stop = False
        new_maze, cur = astar(maze, start, end, stop)
    finally:
        fc.stop()